...
./core/dockero.sh net new "$TEST_NETWORK_NAME" > /dev/null 2>&1
...
INSPECT_OUTPUT=$(./core/dockero.sh net inspect "$TEST_NETWORK_NAME")
...
if ! echo "$INSPECT_OUTPUT" | sed -r "s/\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[m|K]//g" | grep -q "✨ Inspecting Network: $TEST_NETWORK_NAME"; then
    ...
    ./core/dockero.sh net delete "$TEST_NETWORK_NAME" > /dev/null 2>&1
    exit 1
fi
...
./core/dockero.sh net new "$PRUNE_NET1" > /dev/null 2>&1
./core/dockero.sh net new "$PRUNE_NET2" > /dev/null 2>&1

if ./core/dockero.sh net prune; then # Check for successful exit code
...
# if ./core/dockero.sh net create "$CREATE_ALIAS_NET"; then
#     if docker network inspect "$CREATE_ALIAS_NET" &>/dev/null; then
#         echo "✅ net create alias works"
#         ./core/dockero.sh net delete "$CREATE_ALIAS_NET" > /dev/null 2>&1
#     else
...
# ./core/dockero.sh net new "$ALIAS_TEST_NET" > /dev/null 2>&1
# docker run -d --name "$ALIAS_TEST_CONTAINER" alpine:latest sleep 30 > /dev/null 2>&1

# # Connect
# if ./core/dockero.sh net connect "$ALIAS_TEST_CONTAINER" "$ALIAS_TEST_NET" > /dev/null 2>&1; then
#     if ./core/dockero.sh net inspect "$ALIAS_TEST_NET" | grep -q "$ALIAS_TEST_CONTAINER"; then
...
#         ./core/dockero.sh net delete "$ALIAS_TEST_NET" > /dev/null 2>&1
#         exit 1
#     fi
# else
...
# # Disconnect
# if ./core/dockero.sh net disconnect "$ALIAS_TEST_CONTAINER" "$ALIAS_TEST_NET" > /dev/null 2>&1; then
#     if ! ./core/dockero.sh net inspect "$ALIAS_TEST_NET" | grep -q "$ALIAS_TEST_CONTAINER"; then
