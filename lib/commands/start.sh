...
start() {
  ...
  if ${DOCKERO_RUNTIME:-docker} ps -a --format '{{.Names}}' | grep -q "^$container_name$"; then
    ...
    if ${DOCKERO_RUNTIME:-docker} ps --format '{{.Names}}' | grep -q "^$container_name$"; then
        ...
        if ${DOCKERO_RUNTIME:-docker} start "$container_name" > /dev/null; then
    ...
    if [[ -n "$custom_command_flag" && ${#custom_command_args[@]} -gt 0 ]]; then
        ...
        local exec_exit=0
        if [[ -t 0 ]]; then
            ${DOCKERO_RUNTIME:-docker} exec -it "$container_name" "${custom_command_args[@]}" || exec_exit=$?
        else
            ${DOCKERO_RUNTIME:-docker} exec "$container_name" "${custom_command_args[@]}" || exec_exit=$?
        fi
